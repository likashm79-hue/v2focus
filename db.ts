...
export async function createEnquiry(enquiry: InsertEnquiry) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    const result = await db.insert(enquiries).values(enquiry);
    return result;
  } catch (error) {
...
export async function getEnquiries() {
  ...
  try {
    const result = await db.select().from(enquiries).orderBy(enquiries.createdAt);
    return result;
  } catch (error) {
